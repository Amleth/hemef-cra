...
WHERE {
    GRAPH <http://data-iremus.huma-num.fr/graph/hemef> {
        ...
        OPTIONAL { <${
          RESOURCE_PREFIX + id
        }> hemef:discipline_TDC ?discipline_TDC . }
        OPTIONAL { <${
          RESOURCE_PREFIX + id
        }> hemef:discipline_catégorie ?discipline_catégorie . }
        OPTIONAL { <${
          RESOURCE_PREFIX + id
        }> hemef:discipline_catégorie_TDC ?discipline_catégorie_TDC . }
        OPTIONAL { <${RESOURCE_PREFIX + id}> hemef:nom ?nom . }
        OPTIONAL { <${RESOURCE_PREFIX + id}> hemef:nom_TDC ?nom_TDC . }
        OPTIONAL { <${
          RESOURCE_PREFIX + id
        }> hemef:nom_professeur ?nom_professeur . }
        OPTIONAL { <${
          RESOURCE_PREFIX + id
        }> hemef:nom_professeur_TDC ?nom_professeur_TDC . }
        OPTIONAL { <${RESOURCE_PREFIX + id}> hemef:type ?type . }
        OPTIONAL { <${RESOURCE_PREFIX + id}> hemef:type_TDC ?type_TDC . }
        OPTIONAL { <${
          RESOURCE_PREFIX + id
        }> hemef:observations ?observations . }
        OPTIONAL { <${
          RESOURCE_PREFIX + id
        }> hemef:observations_TDC ?observations_TDC . }
        OPTIONAL { <${
          RESOURCE_PREFIX + id
        }> hemef:remarques_saisie ?remarques_saisie . }
    }
}
`;
  ...
  return Object.entries(classe).length === 0 ? (
    ...
    <Container>
      ...
      <MaterialTable
        components={{
          Container: (props) => (
            <Paper {...props} elevation={0} square={true} variant="outlined" />
          ),
        }}
        columns={[
          {
            customFilterAndSearch: (term, rowData) =>
              makeNom(rowData).indexOf(term) !== -1,
            defaultSort: "asc",
            field: `nom.value`,
            render: (r) => makeNom(r),
            title: `Nom`,
          ...
          {
            customFilterAndSearch: (term, rowData) =>
              makePrénom(rowData).indexOf(term) !== -1,
            field: `prénom`,
